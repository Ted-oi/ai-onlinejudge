...
const excelUpload = multer({ dest: 'uploads/temp', limits: { fileSize: 10 * 1024 * 1024 } })
...
export const downloadObjectiveTemplate = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const headers = [
...
export const importObjectiveExcel = async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.file) {
    ...
    if (dataRows.length === 0) {
      return res.status(400).json({ success: false, error: { message: 'Excel 中没有有效数据' } })
    }
