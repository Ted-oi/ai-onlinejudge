...
export const getProblemSets = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.json({
      success: true,
      data: { problemSets: setsWithProgress, total },
    })
  } catch (error) {
    next(error)
  }
...
export const getProblemSetById = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ success: false, error: { message: '题单不存在' } })
    }
...
export const createProblemSet = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(201).json({ success: true, data: { problemSet: result.rows[0] } })
  } catch (error) {
    next(error)
  }
}

export const updateProblemSet = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params
    ...
    res.json({ success: true, data: { problemSet: result.rows[0] } })
  } catch (error) {
    next(error)
  }
}

export const deleteProblemSet = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params
...
export const publishProblemSet = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params
