...
export const getTeams = asyncHandler(async (req, res) => {
  ...
  return sendSuccess(res, { teams: result.rows, total: parseInt(countRes.rows[0].total) })
})
...
export const getMyTeams = asyncHandler(async (req, res) => {
  ...
  return sendSuccess(res, { teams: result.rows })
})
...
export const getTeamById = asyncHandler(async (req, res) => {
  ...
  if (result.rows.length === 0) throw notFound('团队不存在')
...
export const createTeam = asyncHandler(async (req, res) => {
  ...
  if (!name) throw badRequest('团队名称为必填')
  if (team_type === 'class' && userRole !== 'admin' && userRole !== 'teacher') {
    throw forbidden('仅教师/管理员可创建班级')
  }
  ...
  return sendSuccess(res, { team: result.rows[0] }, 201)
})
...
export const updateTeam = asyncHandler(async (req, res) => {
  ...
  if (team.rows.length === 0) throw notFound('团队不存在')
  if (team.rows[0].leader_id !== userId && userRole !== 'admin') throw forbidden('权限不足')
  ...
  return sendSuccess(res, { team: result.rows[0] })
})
...
export const joinTeam = asyncHandler(async (req, res) => {
  ...
  return sendSuccess(res, { message: '已加入团队' })
})
...
export const leaveTeam = asyncHandler(async (req, res) => {
  ...
  if (member.rows.length === 0) throw notFound('未加入此团队')
  if (member.rows[0].role === 'leader') throw badRequest('队长需先转让队长身份')

  await query('DELETE FROM team_members WHERE team_id = $1 AND user_id = $2', [id, userId])
  return sendSuccess(res, { message: '已退出团队' })
})
...
export const removeMember = asyncHandler(async (req, res) => {
  ...
  return sendSuccess(res, { message: '已移除' })
})
...
export const transferLeadership = asyncHandler(async (req, res) => {
  ...
  const team = await query('SELECT leader_id FROM teams WHERE id = $1', [id])
  if (team.rows.length === 0) throw notFound('团队不存在')
  if (team.rows[0].leader_id !== userId) throw forbidden('仅队长可转让')

  const target = await query('SELECT 1 FROM team_members WHERE team_id = $1 AND user_id = $2', [id, new_leader_id])
  if (target.rows.length === 0) throw notFound('目标用户不是成员')
  ...
  return sendSuccess(res, { message: '已转让' })
})
...
export const generateInvite = asyncHandler(async (req, res) => {
  ...
  return sendSuccess(res, { invite_code: code })
})
...
export const getTeamLeaderboard = asyncHandler(async (req, res) => {
  ...
  return sendSuccess(res, { leaderboard: result.rows })
})

export const getGlobalTeamLeaderboard = asyncHandler(async (req, res) => {
  ...
  return sendSuccess(res, { leaderboard: paged, total: leaderboard.length })
})
...
export const joinByCode = asyncHandler(async (req, res) => {
  ...
  return sendSuccess(res, { message: '已加入团队', team_id: t.id })
})
