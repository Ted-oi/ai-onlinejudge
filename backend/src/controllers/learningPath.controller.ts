...
export const getLearningPaths = asyncHandler(async (req, res) => {
  ...
  return sendSuccess(res, { paths: pathsWithStatus, total: parseInt(countRes.rows[0].total), page: parseInt(page as string) })
})
...
export const getLearningPathById = asyncHandler(async (req, res) => {
  ...
  if (pathRes.rows.length === 0) throw notFound('路径不存在')
...
export const createLearningPath = asyncHandler(async (req, res) => {
  ...
  if (!title || !category) throw badRequest('标题和分类为必填')
  ...
  return sendSuccess(res, { path: pathRes.rows[0] }, 201)
})
...
export const updateLearningPath = asyncHandler(async (req, res) => {
  ...
  if (result.rows.length === 0) throw notFound('路径不存在')
  return sendSuccess(res, { path: result.rows[0] })
})
...
export const enrollInPath = asyncHandler(async (req, res) => {
  const { id } = req.params
  const userId = req.userId
  ...
  return sendSuccess(res, { message: '已加入学习路径' })
})

export const unenrollFromPath = asyncHandler(async (req, res) => {
  const { id } = req.params
  const userId = req.userId
  await query('DELETE FROM user_path_enrollments WHERE user_id = $1 AND path_id = $2', [userId, id])
  return sendSuccess(res, { message: '已退出' })
})

export const getMyEnrolledPaths = asyncHandler(async (req, res) => {
  const userId = req.userId
  const result = await query(
...
export const getNextProblemSuggestion = asyncHandler(async (req, res) => {
  const userId = req.userId
  const result = await getNextProblem(userId)
  return sendSuccess(res, result)
})

export const getRecommendedPath = asyncHandler(async (req, res) => {
  const userId = req.userId
